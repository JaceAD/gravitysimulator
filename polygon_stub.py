...
class Polygon:
    def __init__(self, pos, vel, density, points, color, angle=0, angvel=0):
        ...
        self.moment = self.moment - self.mass * center.mag2()
        ...
        for i in range(len(pp)):
            ...
            if(moment == self.moment):
                print("they are equal. That is good???")
            pass
        print("moment =", moment)
        ...
        for i in range(len(pp)):
            ...
            #perpendicular of the side then normalize
            self.orignormals.append((pp[i] - pp[i-1]).normalized())
    ...
    def check_collision(self, other, result=[]):
        ...
        if other.type == "polygon":            
            ...
            for i in range(len(other.normals)):
                # Fill in
#                nhat = other.normals[i]
#                rother = other.pos + other.points[i]
                pass
                for j in range(len(self.points)):
#                    d = (other.r - side.r) * nhat
#                    if(d > maxd):
#                        maxd = d
#                        maxj = j
#                    if(maxd < overlap):
